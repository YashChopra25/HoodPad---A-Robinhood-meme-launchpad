  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-2.5">
      {metrics.map((metric) => (
        <div
          key={metric.label}
          className="relative overflow-hidden rounded-xl border border-line bg-panel px-4 py-3.5"
        >
          <div className="flex items-center gap-1.5 text-xs text-muted">{metric.label}</div>
          <div className="mono mt-1.5 text-[22px] leading-[1.1] font-semibold tracking-[-0.03em]">
            {metric.value}
          </div>
